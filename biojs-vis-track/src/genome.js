...
tnt.board.genome = function () {
    ...
    var start = function (where) {
        if (where !== undefined) {
            ...
            if (genome_browser.gene() !== undefined) {
                get_gene({
                    species: genome_browser.species(),
    ...
    var split_homologues = function (homologues) {
        ...
        var orthologues = homologues.filter(function (d) {
            return d.type.match(orthoPatt)
        });
        var paralogues = homologues.filter(function (d) {
            return d.type.match(paraPatt)
        });
...
module.exports = exports = tnt.board.genome;
