...
data =  Clazz_newFloatArray (atomCount, 0);
if (isVariable) {
...
if (Clazz_instanceOf (area, Float)) this.viewer.setFloatProperty ("isosurfaceArea", (area).floatValue ());
...
if (Clazz_instanceOf (volume, Float)) this.viewer.setFloatProperty ("isosurfaceVolume", (volume).floatValue ());
...
sarea = (doCalcArea ? "isosurfaceArea = " + (Clazz_instanceOf (area, Float) ? "" + area : J.util.Escape.eAD (area)) : null);
svol = (doCalcVolume ? "isosurfaceVolume = " + (Clazz_instanceOf (volume, Float) ? "" + volume : J.util.Escape.eAD (volume)) : null);
if (s == null) {
...
var potentials =  Clazz_newFloatArray (this.viewer.getAtomCount (), 0);
...
var isAtoms = !(Clazz_instanceOf (tokenValue, J.modelset.BondSet));
if (isAtoms) {
...
atomMap =  Clazz_newIntArray (atomCount + 2, 0);
...
var tok = (Clazz_instanceOf (token, J.script.SV) ? 0 : token.tok);
if (tok == 4) {
...
var dataOut =  Clazz_newFloatArray (isProperty ? this.viewer.getAtomCount () : data3.length, 0);
bsOut =  new JU.BS ();
...
}if (!this.chk) this.viewer.getMinimizer (true).setProperty ("constraint", [aList,  Clazz_newIntArray (n, 0), Float.$valueOf (targetValue)]);
return;
case 1073741905:
...
}, "JU.BS,JU.BS,~S,JU.List,JU.List,JU.M4,JU.List,~B,~B,~A,JU.P3");
...
}var nbins = Clazz_doubleToInt (Math.floor ((f1 - f0) / df + 0.01));
...
var bs = (Clazz_instanceOf (args[0].value, JU.BS) ? args[0].value : this.eval.compareInt (1095761939, 269484436, args[0].asInt ()));
switch (tok) {
case 135266320:
...
ret = (map.length > 0 ? this.viewer.getDihedralMap (map[0]) :  Clazz_newIntArray (0, 0));
} else {
...
q = (Clazz_instanceOf (mean, J.util.Quaternion) ? mean : null);
break;
...
if (isWithinGroup) return mp.addXBs (this.viewer.getGroupsWithin (Clazz_floatToInt (distance), bs));
...
var hsl =  Clazz_newFloatArray (3, 0);
...
switch ($var.tok) {
case 10:
isBonds = (Clazz_instanceOf ($var.value, J.modelset.BondSet));
